...
#   の責任を負わない．
#  
#   $Id: GenTransparentMarshaler.rb 3301 2026-01-13 09:20:12Z okuma-top $
#++
...
module GenTransparentMarshaler
  ...
  def initialize_transparent_marshaler cell_name
    # 他で初期化されてない場合に設定
    @task_priority = 8                        if @task_priority == nil
    @channelCelltype = "tDataqueueOWChannel"  if @channelCelltype == nil
    @TDRCelltype     = "tTDR"                 if @TDRCelltype == nil
    @channelCellName = "#{cell_name}_Channel" if @channelCellName == nil
    @PPAllocatorSize = nil                    if @PPAllocatorSize == nil
    @b_datapumpholder = false                 if @b_datapumpholder == nil
    ...
    @marshaler_celltype_name = "tMarshaler#{marshaler_head_append}_#{@signature.get_global_name}"
    @unmarshaler_celltype_name = "tUnmarshaler#{marshaler_head_append}_#{@signature.get_global_name}"
...
EOT
    ...
    if @b_datapumpholder == false || func_name == :waitForReadDone then
      # channel lock コード
      file.print <<EOT
    /* Channel Lock / */
...
EOT
    ...
    if @b_datapumpholder == false || func_name == :newDataReady then
      # EOP の送出
...
EOT
      ...
      if @b_datapumpholder == false || f_name == :newDataReady then
